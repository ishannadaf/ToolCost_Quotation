...
def Frm_Machining_Master(master):
    ...
    def on_start():
        clear_data()
        TxtName.focus()
        sql1 = f"SELECT * FROM machining_master"
    ...
    def save_data():
        ...
        if name != '' and price != '' and BtnSave['text'] == 'Save':
            sql1 = f"INSERT INTO machining_master (name, price) VALUES ({DATABASE_SYN}, {DATABASE_SYN})"
            ...
            sql1 = f"UPDATE machining_master SET name = {DATABASE_SYN}, price = {DATABASE_SYN} WHERE id = {DATABASE_SYN}"
    ...
    def delete_data():
        if LblMasterId['text'] != '':
            sql1 = f"DELETE FROM machining_master WHERE id = {DATABASE_SYN}"
    ...
    LblHead = Label(machining_master, text='Machining Master', font=('Times New Roman', 22))
    ...
    TxtPrice = Entry(Frm1, font=('Times New Roman', 14), width=20)
    ...
    trv=ttk.Treeview(Frm1 , columns=("no" , "name" , "price") , yscrollcommand=scrolly.set )#, xscrollcommand=scrollx.set

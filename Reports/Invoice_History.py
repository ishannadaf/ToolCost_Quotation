...
def Frm_Invoice_Report(master, login_id):
    def Generate_Report(data, path):
        ...
        # Header row
        headers = [
            "Sr. No.", "Invoice No", "Customer Name", "Provider", "Amount", "Total GST", "Date"]
        ws.append(headers)
    ...
    def Fetch_Data():
        ...
        sql1 = f"""
        select im.invoice_id, am.party_name, qm.provider_name, sum(qmd.total_price) as Amt, (sum(qmd.total_price) * 18 / 100) as Total_GST, qm.date_tr_quot from quotation_master qm 
        ...
        group by im.invoice_id, am.party_name, qm.provider_name, qm.date_tr_quot
        """
